...
CHintBox::CHintBox(const char * Caption, const char * const Text, const int Width, const char * const Icon)
{
	...
	m_cBoxWindow->setShadowMode(g_settings.menu_border);
	m_cBoxWindow->setCorner(g_settings.Head_radius | g_settings.Foot_radius, g_settings.Head_corner | g_settings.Foot_corner);
	
	// head
	cFrameBoxTitle.iX = g_settings.menu_border? cFrameBox.iX + 2 : cFrameBox.iX;
	cFrameBoxTitle.iY = g_settings.menu_border? cFrameBox.iY + 2 : cFrameBox.iY;
	cFrameBoxTitle.iWidth = g_settings.menu_border? cFrameBox.iWidth - 4 : cFrameBox.iWidth;
